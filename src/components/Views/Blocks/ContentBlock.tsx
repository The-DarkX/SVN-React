...
const ContentBlock: React.FC<{ img: string, reverseAlign?: boolean, children?: any, style?: object, id?: string, padding?: string; }> = ({ img, reverseAlign = false, children, style, id, padding = '3rem 2rem' }) => {
    return (
        <div id={id}>
            <Stack direction={!reverseAlign ? 'row' : 'row-reverse'} flexWrap='wrap' spacing={1} alignItems='stretch' textAlign={reverseAlign ? 'end' : 'start'} alignContent='flex-start' justifyContent='space-around' sx={{ ...style, padding }}>
